...
def resample_raster_to_ref(src_path: str, ref_path: str) -> np.ndarray:
    """Resample/reproject a source raster so it matches the grid of a reference raster.

    Returns an array with shape (bands, height, width) in the source raster's dtype.
    """
    ...
    dst_shape = (src_data.shape[0], dst_height, dst_width)
...
def process_satellite(
    ...
    pmin: float = 2,
    pmax: float = 98,
    ref_dtm_path: Optional[str] = None,
) -> None:
    ...
    `scale_to_uint8`.
    """
    # If a DTM reference is provided and exists, resample the satellite to DTM grid
    if ref_dtm_path is not None and Path(ref_dtm_path).exists():
        try:
            data = resample_raster_to_ref(src_path, ref_dtm_path)
        except Exception:
            # fallback to reading without resampling
            with rasterio.open(src_path) as src:
                data = src.read()
    else:
        with rasterio.open(src_path) as src:
    ...
    # Ensure we have at least 3 channels; if single-band replicate, if more than 3 take first 3
    if data.shape[0] >= 3:
        arr = data[:3]
    elif data.shape[0] == 1:
        arr = np.stack([data[0], data[0], data[0]], axis=0)
    else:
        # fallback: compute mean across channels and replicate
...
def main():
    ...
    # Map DTM basenames to their paths so we can resample satellite images
    # to the corresponding DTM grid when available.
    dtm_map = {p.stem: str(p) for p in dtm_files}
    ...
    for p in tqdm(sat_files, desc="SAT"):
        ...
        try:
            # If a DTM with same base name exists, pass it as reference for resampling
            ref = dtm_map.get(name)
            process_satellite(
                str(p),
                str(out_path),
                method=args.method,
